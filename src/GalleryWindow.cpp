...
void UpdateActionButtons(GalleryState* st) {
    ...
    EnableWindow(st->btnExtract, has);
}
...
// Runs OCR over the selected capture and opens the word-selection window on
// it. The bitmap is decoded here rather than borrowed from st->preview:
// the overlay taking focus makes WM_ACTIVATE fire, which can Reload() the
// folder and reset that cache mid-call.
void ExtractTextSelected(GalleryState* st) {
    const int index = SelectedIndex(st);
    if (index < 0) return;

    std::unique_ptr<Bitmap> image(
        Utils::LoadImageFromFile(st->items[index].path));
    if (!image || image->GetLastStatus() != Ok) {
        Toast::Show(L"Extract text failed",
                    L"The capture could not be opened. See app.log for details.");
        return;
    }

    POINT cursor = {};
    GetCursorPos(&cursor);
    const RECT fit = OcrOverlay::FitRegion(image.get(), cursor);

    // Blocks until the word-selection window is done.
    const OcrOverlay::Result ocr = OcrOverlay::Show(fit, image.get());
    CaptureController::DeliverExtractedText(ocr, fit);
}
...
void LayoutChildren(GalleryState* st) {
    ...
    // Widest label in the group, so it gets a little more room.
    placeBottom(st->btnExtract, Scaled(104));
...
void OnCommand(HWND hwnd, int id) {
    ...
    switch (id) {
        ...
        case ID_GAL_EXTRACT:  ExtractTextSelected(st); return;
...
bool Create(HINSTANCE instance) {
    ...
    st->btnExtract = MakeButton(st->hwnd, L"Extract text", ID_GAL_EXTRACT);
