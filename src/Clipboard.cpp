...
bool PutText(const std::wstring& text) {
    if (text.empty()) {
        Logger::Error(L"Clipboard copy failed: there is no text to place");
        return false;
    }

    // Built before OpenClipboard, like the image path: nothing expensive
    // should happen while the clipboard is held open.
    const SIZE_T bytes = (text.size() + 1) * sizeof(wchar_t);
    HGLOBAL hg = GlobalAlloc(GMEM_MOVEABLE, bytes);
    if (!hg) {
        Logger::Errorf(L"Clipboard copy failed: GlobalAlloc (error %lu)",
                       GetLastError());
        return false;
    }
    void* dst = GlobalLock(hg);
    if (!dst) {
        GlobalFree(hg);
        Logger::Errorf(L"Clipboard copy failed: GlobalLock (error %lu)",
                       GetLastError());
        return false;
    }
    memcpy(dst, text.c_str(), bytes);  // includes the terminating L'\0'
    GlobalUnlock(hg);
    ...
    EmptyClipboard();
    // SetClipboardData takes ownership of the handle only when it succeeds.
    const bool ok = SetClipboardData(CF_UNICODETEXT, hg) != nullptr;
    if (!ok) GlobalFree(hg);
    CloseClipboard();

    if (!ok) {
        Logger::Error(L"Clipboard copy failed: CF_UNICODETEXT could not be placed");
        return false;
    }
    return true;
}
