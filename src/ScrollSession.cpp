...
constexpr int kWheelIntervalMinMs = 16;
constexpr int kWheelIntervalMaxMs = 240;
constexpr int kMaxWheelNotches = 4;
...
constexpr DWORD kSpeedReviewMs = 300;
...
// One step along auto's speed ladder. Notches come off first on the way down
// and go on last on the way up, so the page is always scrolled in the smallest
// pulses that will keep up: a big pulse is a jump, and a jump is what the
// matcher has to guess at.
void NudgeScrollSpeed(Session* s, bool faster) {
    ...
    if (faster) {
        if (interval > kWheelIntervalMinMs) {
            interval = (std::max)(kWheelIntervalMinMs, interval * 4 / 5);
        ...
        if (notches > 1) {
            --notches;
        } else if (interval < kWheelIntervalMaxMs) {
            interval = (std::min)(kWheelIntervalMaxMs, interval * 5 / 4);
        } else {
...
//
// Nothing here ever stops the scroll. The only lever this has on it is the
// pulse rate, and that is reviewed a few times a second rather than every
// tick, because a rate that keeps being corrected is indistinguishable from
// the stop-and-go it replaced.
