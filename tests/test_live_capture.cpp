...
TEST(LiveCapture_FadedScrollbarIsAbsorbedByRowTolerance) {
    // The auto-scroll first-seam scenario: the frame before any scrolling
    // has no scrollbar; the one after the first scroll does. A 17px bar on a
    // 500px-wide region is 3.4% of every row - inside the 10% rowMatchFraction
    // lets a row differ by - so the seam survives even without the live
    // path's right-edge exclusion. (This test used to assert the opposite,
    // back when a row could differ by only 3%; relaxing rowMatchFraction to
    // absorb blinking carets and hover highlights absorbed this case too.)
    ...
    const MatchResult m = FindVerticalOverlap(a.View(), b.View(), opt);
    CHECK_MSG(m.matched,
              "a fade-in scrollbar alone should not break the seam any more");
}

TEST(LiveCapture_AWiderScrollbarStillDefeatsTheMatch) {
    // The same scenario with a bar too wide for row tolerance to absorb:
    // 60px of 500 is 12%, so every row fails, the seam is missed entirely,
    // and the frames would be butted together with their overlap duplicated.
    // This is why the live capture path still excludes the strip a scrollbar
    // lives in before matching.
    const Image page = MakeDocumentPage(500, 3000, 909);
    const Image a = WindowOf(page, 0, 600);
    Image b = WindowOf(page, 200, 600);
    PaintScrollbar(b, 60, 100, 260);
    ...
    const MatchResult m = FindVerticalOverlap(a.View(), b.View(), opt);
    CHECK_MSG(!m.matched,
              "a scrollbar wider than row tolerance must still defeat the "
              "match");
}
